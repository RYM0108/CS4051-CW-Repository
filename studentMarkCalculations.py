...
def student_marks():
    ...
    while True:
        ...
        try:  # Run this code to handle errors if they occur
            ...
            string_list = user.replace(" ", "").split(
                ",")  # Manipulate the string by replacing space string with a empty string before splitting. Split the string to cut unwanted commas with the delimiter as ","
...
def calculate_median():
    ...
    if len(marks) % 2 == 1:
        ...
        return marks[
            int(median) - 1]  # Use marks[int(median) -1] to get the element inside the array, -1 is used because the index starts at 0
...
#Open the data.txt file to write content and automatically close it after use
with open("data.txt", "w") as file:
    #Perform file operations such as write() to insert content into the text file
    #Format the text data to make it look clean using f"" and \n (newline)
    file.write(f"\nStudent Marks: {marks}") #Pass marks as an argument into the write() function to overwrite data on the file
    file.write(f"\nMean: {calculate_mean()}") #Pass the mean function result into write() function to overwrite data on the file
    file.write(f"\nMedian: {calculate_median()}") #Pass the median function result into write() function to overwrite data on the file
    file.write(f"\nMode: {calculate_mode()}") #Pass the mode function result into write() function to overwrite data on the file
    file.write(f"\nStandard Deviation: {standard_deviation()}") #Pass the SD function result into write() function to overwrite data on the file
    file.write(f"\nSkewness:{skewness()}") #Pass the skewness function result into write() function to overwrite data on the file

#Read the data.txt file on the terminal window once the text file has content inside
with open("data.txt", "r") as f:
    content = f.read() #Create a variable and use the method read() to read every text data inside the file
    print(content) #Print out the text data onto the terminal window
